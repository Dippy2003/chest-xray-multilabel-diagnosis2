"""
Train EfficientNet-B0 transfer learning model.

Usage:
    python scripts/train_efficientnet.py [--epochs N] [--lr LR]
"""
import argparse
import json
import sys
...
def main():
    ...
    model = EfficientNetTransfer(num_classes=len(CLASS_NAMES)).to(device)
    ...
    history = []
    checkpoint_path = MODELS_DIR / "efficientnet_b0_best.pt"
    ...
    pd.DataFrame(history).to_csv(METRICS_DIR / "efficientnet_b0_history.csv", index=False)
    ...
    with open(METRICS_DIR / "efficientnet_b0_test_metrics.json", "w") as f:
